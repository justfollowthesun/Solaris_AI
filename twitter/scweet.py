...
from nlp_functions.text_classification import text_tonality
...
def scrape(since, until=None, words=None, to_account=None, from_account=None, mention_account=None, interval=5, lang=None,
          ...
          show_images=False, save_images=False, save_dir=".", filter_replies=False, proximity=False, 
          geocode=None, minreplies=None, minlikes=None, minretweets=None,verbose=False):
    """
    ...
    with open(path, write_mode, newline='', encoding='utf-8') as f:
        ...
        while until_local <= datetime.datetime.strptime(until, '%Y-%m-%d'):
            ...
            driver, data, writer, tweet_ids, scrolling, tweet_parsed, scroll, last_position = \
                keep_scroling(driver, data, writer, tweet_ids, scrolling, tweet_parsed, limit, scroll, last_position,verbose)
    ...
    data = pd.DataFrame(data, columns = ['tweet_id','UserScreenName', 'UserName', 'Timestamp', 'Text', 'Embedded_text', 'Emojis', 
    ...
    driver.close()
    data['ton']= data.Embedded_text.apply(text_tonality)
    dict_ = data.set_index('tweet_id').T.to_dict()
    with open(words[0]+'_all_mentions.json','w') as fp:
